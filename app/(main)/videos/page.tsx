...
import { allVideos, featuredVideos, fullVideos, seoShorts, Video, videoCategoryMap } from '@/lib/video-data';
...
export default function VideosPage() {
  const categories = Array.from(videoCategoryMap.keys()).sort();
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            ...
            {categories.map((cat) => (
              <a key={cat} href={`#${cat.toLowerCase().replace(/\s+/g, '-')}`}
                ...
                {cat} ({videoCategoryMap.get(cat)?.length || 0})
              </a>
            ))}
          </div>
