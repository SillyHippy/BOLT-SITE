...
## 2025-10-25 - Shift O(N log N) static calculations to O(1) Lookups
**Learning:** For computationally expensive operations on static data (e.g., calculating haversine distances and sorting items in `lib/county-neighbors.ts`), performing the calculations repeatedly during renders or static generation causes performance bottlenecks.
